...
contract Crowdsale is Ownable {
  ...
  function Crowdsale(MintableToken tokenContract, uint256 _startBlock, uint256 _endBlock, uint256 _rate, address _wallet) {
    ...
    //token.transferFrom (msg.sender,this,_limitTokensToSale);
    //token.transfer(beneficiary, tokens);
    //token.approve(this, limitTokensToSale);
  }
  ...
  function buyTokens(address beneficiary) payable {
    ...
    //require ( tokensSold.add(tokens) <= limitTokensToSale ); // check for tokens sold limit
     require ( tokens <= token.balanceOf(this) ); // check for tokens enoight
  ...
  function validPurchase() internal constant returns (bool) {
    ...
    //bool maxTokensSold = tokensSold < limitTokensToSale;    // check limit for sold tokens count
    return withinPeriod && nonZeroPurchase;// && maxTokensSold;
  }

  // @return true if crowdsale event has ended
  function hasEnded() public constant returns (bool) {
    return ( block.number > endBlock );// || (tokensSold >= token.balanceOf(this)) ;
  ...
  {
     // require (hasEnded());
       //token.transfer(owner, limitTokensToSale - tokensSold);
